...
export const create = createAsyncThunk('surveys/create', async () => {
    // Create a new survey on the server; backend supplies default JSON
    const response = await axios.post(apiBaseAddress + '/create')
    return response.data
})

export const remove = createAsyncThunk('surveys/delete', async (id: string) => {
    const response = await axios.post(apiBaseAddress + '/delete', { id })
    return response.data
})
